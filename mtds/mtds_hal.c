...
void __iomem* spi_map_base;
void __iomem* gpio_map_base;
void __iomem* timer_map_base;
...
{
    printk(KERN_INFO "MtdsHalInit");

    spi_map_base = ioremap(XPAR_PMODMTDS_0_AXI_LITE_SPI_BASEADDR, SZ_4K);
    gpio_map_base = ioremap(XPAR_PMODMTDS_0_AXI_LITE_GPIO_BASEADDR, SZ_4K);
    timer_map_base = ioremap(XPAR_PMODMTDS_0_AXI_LITE_TIMER_BASEADDR, SZ_4K);

    Xil_Out32(gpio_map_base+4, 0b11);//Set IntA and IntB to inputs
    Xil_Out32(gpio_map_base+8, 0b111);
    Xil_Out32(gpio_map_base+12, 0b000);//Set chip selects(ch2) to outputs
}
...
{
	Xil_Out32(gpio_map_base+4,0b11);//Set interrupts (ch1) as inputs
}
...
{
    printk(KERN_INFO "MtdsHalInitSpi");

	MTDSSPIConfig.BaseAddress = (UINTPTR) spi_map_base;

	int result = XSpi_CfgInitialize(&MTDS_Spi, &MTDSSPIConfig, (UINTPTR)spi_map_base);

	if(result != XST_SUCCESS && result != XST_DEVICE_IS_STARTED) {
        printk(KERN_INFO "Unable to initialize SPI controller.");
        return;
	}
...
void MtdsHalEnableSlave(bool fEn) {
    ...
    if (fEn)
    {
		Xil_Out32(gpio_map_base+8, Xil_In32(gpio_map_base+8)&0b011);
	}
    else
    {
		Xil_Out32(gpio_map_base+8, Xil_In32(gpio_map_base+8)|0b100);
	}
}
...
uint32_t MtdsHalTmsElapsed(void)
{
	return 0;
}
